...
local SelectionBox = Create("SelectionBox",Parent,{
	["Color"] = BrickColor.new("Bright blue")
})

local Handle = Create("ArcHandles",Parent,{
	["Color"] = BrickColor.new("Bright orange")
})

local MoveHandle = Create("Handles",Parent,{
	["Style"] = Enum.HandlesStyle.Movement
})

Events["MouseUp"] = mouse.Button1Up:connect(function()
	if Rotating then
		Handle.Axes = Axes.new(Enum.Axis.X,Enum.Axis.Y,Enum.Axis.Z)
		MoveHandle.Adornee = nil
	else
		Handle.Axes = Axes.new()
		MoveHandle.Faces = Faces.new(Enum.NormalId.Right,Enum.NormalId.Left,Enum.NormalId.Bottom,Enum.NormalId.Top,Enum.NormalId.Back,Enum.NormalId.Front)
		MoveHandle.Adornee = Handle.Adornee
	end
end)

Events["HandleMouseDown"] = Handle.MouseButton1Down:connect(function(axis)
	if Selected ~= 0 then
		if Rotating then
			lastCFrame = Welds[Selected][2][string.sub(tostring(axis),11).. "1"]
			Handle.Axes = Axes.new(axis)
		end
	end
end)

Events["HandleMouseDrag"] = Handle.MouseDrag:connect(function(axis, relativeAngle, deltaRadius)
	if Selected ~= 0 then
		if Rotating then
			local a = roundDecimals(lastCFrame + relativeAngle*Increment,1)
			Welds[Selected][2][string.sub(tostring(axis),11).. "1"] = a
			MasterModify[string.sub(tostring(axis),11).. "1"].Text = tostring(roundDecimals(a,2))
		end
	end
end)

Events["MoveHandleMouseDown"] = MoveHandle.MouseButton1Down:connect(function(axis)
	if Selected ~= 0 then
		if not Rotating then
			lastCFrame = Welds[Selected][2][AxisToDirection(axis)]
			MoveHandle.Faces = Faces.new(axis)
		end
	end
end)

Events["MoveHandleMouseDrag"] = MoveHandle.MouseDrag:connect(function(face,distance)
	if Selected ~= 0 then
		if not Rotating then
			local a = lastCFrame + distance
			Welds[Selected][2][AxisToDirection(face)] = a*IsNegative(face)
			MasterModify[AxisToDirection(face)].Text = tostring(roundDecimals(a*IsNegative(face),2))
		end
	end
end)
...
ExportButton.Activated:Connect(function()
	...
	if not isfile(InputBox.Text .. ".anim") then
		...
		writefile(InputBox.Text .. ".anim",MasterString)
	end
end)
