...
Last Modified: 12/31/2021 7:06AM
Author: ???
...
--
_G.GodMode = false
_G.Fling = false -- don't change this
--
_G.FakeGod = true

local flingedition = false
...
local flingpos = CFrame.new(0,-5,0)
...
char[plr.Name]["RockAccessory"].Handle.SpecialMesh:Destroy()
end
local purple,red
...
local scarfs = {purple,red,yellow,white,grey,cyan}
...
grey.Anchored = false
grey.AccessoryWeld.Part1 = grey.OrigWeld.Value
...
mouse.Button1Down:connect(function(key)
	if attack == false then
		...
		if combo == 1 and wait2 == false then
			char[plr.Name]["Right Arm"].BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
			wait2 = true
			for i = 0, 1.2, 0.1 do
				swait()
			...
			combo = 2
			char[plr.Name]["Right Arm"].BodyAngularVelocity.AngularVelocity = Vector3.new(0,0,0)
		end
		...
		if combo == 3 and wait2 == false then
			char[plr.Name]["Right Leg"].BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
			char[plr.Name]["Left Leg"].BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
			for i = 0, 1.2, 0.1 do
				swait()
				rootj.C0 = clerp(rootj.C0, RootCF * CFrame.new(0, -0.5, 0) * angles(math.rad(-5), math.rad(0), math.rad(-35)), 0.3)
				tors.Neck.C0 = clerp(tors.Neck.C0, necko * angles(math.rad(-10), math.rad(0), math.rad(0)), 0.1)
			...
			combo = 1
			char[plr.Name]["Right Leg"].BodyAngularVelocity.AngularVelocity = Vector3.new(0,0,0)
			char[plr.Name]["Left Leg"].BodyAngularVelocity.AngularVelocity = Vector3.new(0,0,0)
		end
		hum.WalkSpeed = 8
		wait2 = false
		attack = false
...
function Flying_Kick()
	...
	bodyVelocity2.velocity = Vector3.new(0, 0, 0) + root.CFrame.lookVector * 250
	bodyVelocity2.P = 5000
	bodyVelocity2.maxForce = Vector3.new(9e99, 9e99, 9e99)
...
function Diablo_Explosion()
	char[plr.Name]["Right Arm"].BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
	char[plr.Name]["Left Arm"].BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
	attack = true
	hum.WalkSpeed = 3.01
	for i = 0,6.3,0.1 do
		swait()
	...
	attack = false
	char[plr.Name]["Right Arm"].BodyAngularVelocity.AngularVelocity = Vector3.new(0,0,0)
	char[plr.Name]["Left Arm"].BodyAngularVelocity.AngularVelocity = Vector3.new(0,0,0)
	hum.WalkSpeed = 8
end
...
mouse.KeyDown:connect(function(key)
	if attack == false then
		...
		elseif key == 'e' and EdgyMode == false and Divinity == false and Fighter == false and Purity == false and Destruction == false and helpme == false and Planetary == false then
			...
			Music.SoundId = "rbxassetid://1280010741"
			Music.Parent = tors
			Music:Play()
			for i,v in pairs(scarfs) do
				local origweld = Instance.new("ObjectValue",v)
		...
		elseif key == 'f' and EdgyMode == false and Divinity == false and Fighter == false and Purity == false and Destruction == false and helpme == false and Planetary == false then
			...
			Music.SoundId = "rbxassetid://196678198"
			Music.Parent = tors
			Music:Play()
			for i,v in pairs(scarfs) do
				local origweld = Instance.new("ObjectValue",v)
...
if _G.FakeGod then
	local fakeheadthingy = char[plr.Name]
	local selectionbox = Instance.new("SelectionBox",fakeheadthingy.Torso)
	fakeheadthingy.Torso.BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
	selectionbox.Adornee = fakeheadthingy.Torso; selectionbox.Color3 = Color3.new(1,0,0)
	game["Run Service"].Heartbeat:Connect(function(delta)
        fakeheadthingy.Torso.CFrame = fakeheadthingy.Parent.Torso.CFrame* CFrame.new(0,-10,0)
        if not fakeheadthingy.Torso:FindFirstChild("Neck") then
            fakeheadthingy.Head.CFrame = fakeheadthingy.Parent.Head.CFrame* CFrame.new(0,-10,0)
        end
    end)
end
if _G.Fling then
	local selectionbox = Instance.new("SelectionBox",char[plr.Name].HumanoidRootPart)
	selectionbox.Adornee = selectionbox.Parent; selectionbox.Color3 = Color3.new(1,0,0)
	selectionbox.Parent.BodyAngularVelocity.AngularVelocity = Vector3.new(311111,311111,311111)
	game["Run Service"].RenderStepped:Connect(function(delta)
		selectionbox.Parent.CFrame = flingpos
	end)
end
...
game["Run Service"].Heartbeat:Connect(function(delta)
	...
	--game:GetService("TweenService"):Create(arm1,TweenInfo.new((delta)),{CFrame = char["Right Arm"].CFrame * CFrame.Angles(0,0,math.rad(180)) * CFrame.new(0,0.6,0) }):Play()
	--game:GetService("TweenService"):Create(arm2,TweenInfo.new((delta)),{CFrame = char["Right Arm"].CFrame}):Play()
	arm1.CFrame = char["Right Arm"].CFrame * CFrame.Angles(0,0,math.rad(180)) * CFrame.new(0,0.6,0)
	arm2.CFrame = char["Right Arm"].CFrame
if trail then
	trail.CFrame = char["Right Arm"].CFrame * CFrame.new(0,-0.3,0)
end
	for i,v in pairs(scarfs) do
		--game:GetService("TweenService"):Create(v,TweenInfo.new((delta)),{CFrame = char["Torso"].CFrame * CFrame.new(0,-5,0)}):Play()
...
if boombox then
	...
	char[plr.Name][boombox.Name].Remote:FireServer("PlaySong", "1117396305")
	boombox.Handle.Anchored = true
	ra["RightGrip"]:Destroy()
end

Music:GetPropertyChangedSignal("SoundId"):Connect(function()
	char[plr.Name][boombox.Name].Remote:FireServer("PlaySong", string.match(tostring(Music.SoundId) , "%d+"))
end)	
...
while true do
	swait()
	for i,v in pairs(char[plr.Name]:GetChildren()) do
		if v:IsA("BasePart") then
			v.CanCollide = false
			if not flingedition then
	...
	if not attack then
		flingpos = char["Torso"].CFrame + Vector3.new(0,-5,0)
	end
	Music.Parent = char
	if boombox then
		Music.Volume = 0
